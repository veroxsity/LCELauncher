...
public sealed class BridgeRuntimeManager
{
    ...
    public string StatusText => _activeBridge is null
        ? "No active bridge"
        ...
        CancellationToken cancellationToken)
    {
        ...
        var javaExe = string.IsNullOrWhiteSpace(config.JavaExecutablePath) ? "java" : config.JavaExecutablePath.Trim();
        var bridgeJar = config.BridgeJarPath?.Trim();
        if (string.IsNullOrWhiteSpace(bridgeJar))
            throw new InvalidOperationException("Bridge jar path is not configured.");
        if (!File.Exists(bridgeJar))
            throw new FileNotFoundException("Bridge jar was not found.", bridgeJar);

        var runtimeDir = _paths.GetBridgeRuntimeDirectory(server.Id);
        Directory.CreateDirectory(runtimeDir);

        var configPath = Path.Combine(runtimeDir, "config.yml");
        ...
        startInfo.ArgumentList.Add(bridgeJar);
        ...
        _activeBridge = new ManagedBridgeProcess(
            server.Id, server.DisplayName, server.LocalBridgePort.Value, runtimeDir, configPath, process);
        ...
        if (!await WaitForPortAsync(server.LocalBridgePort.Value, process, cancellationToken))
        {
            await StopAsync();
    ...
    {
        if (_activeBridge is null) return;
        var bridge = _activeBridge;
        _activeBridge = null;
        if (bridge.Process.HasExited) return;
    ...
    private void AttachLogging(Process process, string displayName)
    {
        process.OutputDataReceived += (_, e) =>
        {
            if (!string.IsNullOrWhiteSpace(e.Data))
                _logger.Info($"[Bridge:{displayName}] {e.Data}");
        };
        process.ErrorDataReceived += (_, e) =>
        {
            if (!string.IsNullOrWhiteSpace(e.Data))
                _logger.Warn($"[Bridge:{displayName}] {e.Data}");
        };
    ...
    private static async Task<bool> WaitForPortAsync(int port, Process process, CancellationToken cancellationToken)
    {
        ...
        while (!linked.IsCancellationRequested)
        {
            ...
            {
                try { await Task.Delay(250, linked.Token); }
    ...
    private sealed record ManagedBridgeProcess(
        string ServerId, string ServerDisplayName, int LocalPort,
        string RuntimeDirectory, string ConfigPath, Process Process);
}
